...
class TestTransitionTrajectory(unittest.TestCase):
    ...
    def test_valid_walk_small_gait(self):
        ...
        is_walk_small_gait = False if self.gait_selection[walk_small] is None else True
    ...
    def test_valid_walk_medium_gait(self):
        # check if gait name exists
        is_walk_medium_medium = False if self.gait_selection[walk_medium] is None else True
    ...
    def test_right_open_walk_small(self):
        ...
        is_right_open_subgait = False if self.gait_selection[walk_small][right_open] is None else True
    ...
    def test_right_open_walk_medium(self):
        # check if subgait name exists
        is_right_open_subgait = False if self.gait_selection[walk_medium][right_open] is None else True
        self.assertTrue(is_right_open_subgait, msg='{sg} subgait could not be found in gait {gn}'
                        .format(sg=right_open, gn=walk_medium))

    def test_right_swing_in_walk_small(self):
        # check if subgait name exists
        is_right_swing_subgait = False if self.gait_selection[walk_small][right_swing] is None else True
    ...
    def test_right_swing_in_walk_medium(self):
        # check if subgait name exists
        is_right_swing_subgait = False if self.gait_selection[walk_medium][right_swing] is None else True
        self.assertTrue(is_right_swing_subgait, msg='{sg} subgait could not be found in gait {gn}'
                        .format(sg=right_swing, gn=walk_medium))

    def test_right_close_walk_small(self):
        # check if subgait name exists
        is_right_close_subgait = False if self.gait_selection[walk_small][right_close] is None else True
    ...
    def test_right_close_walk_medium(self):
        # check if subgait name exists
        is_right_close_subgait = False if self.gait_selection[walk_medium][right_close] is None else True
        self.assertTrue(is_right_close_subgait, msg='{sg} subgait could not be found in gait {gn}'
    ...
    def test_valid_transition_small_to_medium_right_open(self):
        ...
        TransitionSubgait.from_subgait_names(self.gait_selection, walk_small, walk_medium, right_open)

    def test_valid_transition_medium_to_small_right_open(self):
        #  Test if the TransitionSubgait is created without an error
        TransitionSubgait.from_subgait_names(self.gait_selection, walk_medium, walk_small, right_open)
    ...
    def test_valid_transition_small_to_medium_right_close(self):
        #  Test if the TransitionSubgait is created without an error
        TransitionSubgait.from_subgait_names(self.gait_selection, walk_small, walk_medium, right_close)

    def test_valid_transition_medium_to_small_right_close(self):
        #  Test if the TransitionSubgait is created without an error
        TransitionSubgait.from_subgait_names(self.gait_selection, walk_medium, walk_small, right_close)
